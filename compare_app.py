...
def compare_sheets_by_position(new_df, old_df, sheet_name, id_col='nrBtsID'):
    differences = []

    # Ensure the identifier column exists in new_df
    if id_col not in new_df.columns:
        st.error(f"❌ العمود '{id_col}' غير موجود في البيانات. تأكد من وجوده.")
        return differences
    ...
    for row in range(min_rows):
        for col in range(min_cols):
            ...
            if pd.isna(new_val) and pd.isna(old_val):
                continue
            if new_val != old_val:
                # Use nrBtsID as reference for the row
                bts_id = new_df.iloc[row][id_col]
                differences.append({
                    "Sheet": sheet_name,
    ...
    if st.button("🔍 قارن الملفات"):
        try:
            ...
            if all_diffs:
                ...
                st.success("✅ تم استخراج الفروقات بين الخلايا مع عمود nrBtsID")
