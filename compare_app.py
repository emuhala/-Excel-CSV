...
# Function to compare sheets by position including nrBtsID reference column
def compare_sheets_by_position(new_df, old_df, sheet_name, id_col=None):
    differences = []

    # Auto-detect identifier column (case-insensitive)
    candidates = [col for col in new_df.columns if col.lower() == 'nrbtsid']
    if not candidates:
        st.error("❌ العمود 'nrBtsID' غير موجود في البيانات. تأكد من وجوده أو تسميته بشكل صحيح.")
        return differences
    id_col_detected = candidates[0]
    ...
    for row in range(min_rows):
        for col in range(min_cols):
            ...
            if pd.isna(new_val) and pd.isna(old_val):
                continue
            if new_val != old_val:
                bts_id = new_df.iloc[row][id_col_detected]
                differences.append({
                    "Sheet": sheet_name,
                    "Row": row + 2,  # Excel row number (header+1)
                    "nrBtsID": bts_id,
                    "Column": columns[col],
                    "Old Value": old_val,
                    "New Value": new_val
...
st.set_page_config(page_title="مقارنة Excel و CSV مع معرف NR BTS", layout="centered")
st.title("📊 مقارنة كل خلية في ملفات Excel أو CSV مع إضافة عمود nrBtsID")
    ...
    if st.button("🔍 قارن الملفات"):
        try:
            ...
            if all_diffs:
                ...
                st.download_button("⬇️ تحميل تقرير الفروقات", open(output_path, "rb"), file_name="Cell_Differences_Report.xlsx")
            else:
